...
function parseBeers(data: string): Beer[] {
    ...
    let parseCategory = (category: cheerio.Element) : Beer[] => {
        let categoryName = 
            $('.tap-list-category-block-title', category).text().trim()
        let beers = 
            $('.tap-list-beer', category)
                .toArray()
    ...
    let categories = 
        $('.the-tap-list div.tap-list-beer-category-block').toArray()
    
    return categories
        .filter(isBeerCategory)
        .flatMap(parseCategory)    
}

function removeUndefined(input: (string | undefined)[]) : string[] {
    const result : string[] = []
    input.forEach(x => {
        if (x) {
            result.push(x)
